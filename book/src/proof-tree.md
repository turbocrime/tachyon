...
for $K$ the window width. The sequence is committed before $z$ exists, and every factor's scalars are pinned in-circuit: each epoch index is `base` plus a constant, and each nullifier is a sponge output of the threaded `mk`. `group` is a free witness, pinned by the header it produces because `base` is injective in it.
...
| NfDerive | NfMasterHeader | — | group, seq | NullifierDerivation |
