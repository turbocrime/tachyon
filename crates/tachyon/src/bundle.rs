...
impl Plan {
    ...
    #[must_use]
    pub fn derive_bsk_private(&self) -> custody::BindingSigningKey {
        let trapdoors: Vec<_> = self.actions.iter().map(|plan| plan.rcv).collect();
        custody::BindingSigningKey::from(trapdoors.as_slice())
    }
}
...
mod tests {
    ...
    fn build_test_bundle(rng: &mut (impl RngCore + CryptoRng)) -> Stamped {
        ...
        let output_rsk = custody::ActionSigningKey::new(output_alpha);
