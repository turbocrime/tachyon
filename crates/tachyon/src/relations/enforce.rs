...
//!
//! # Pinning a free scalar
//!
//! A step that checks an identity in a scalar it witnesses freely, rather
//! than one taken from a header, must fix that scalar before the challenge
//! exists. The discipline is to absorb a *binding point* for it, an
//! injective in-circuit function of the scalar such as $G_i \cdot x$ against
//! a host generator, alongside the operand commitments in
//! [`StepCtx::derive_challenge`]. Several free scalars ride distinct
//! generators, $\sum_i G_i \cdot x_i$, which is injective in the tuple.
//! The identity at the resulting challenge then pins the scalar as it pins
//! the operands, and a step needs only to name which of its scalars are free
//! and which points it absorbs.
//!
