//! Custody keys — signing device only, never delegated.
//!
//! Keys in this module never leave the signing device (cold wallet, HSM).
//! Compromise means fund loss.
...
use super::{
    delegated,
    private::{self, NullifierKey},
    public::{self, PaymentKey},
};
use crate::{
    action, bundle,
    constants::PrfExpand,
...
impl SpendingKey {
    ...
    #[must_use]
    pub fn derive_proof_private(&self) -> delegated::ProofAuthorizingKey {
        let ak = self.derive_auth_private().derive_auth_public();
        let nk = self.derive_nullifier_private();
        delegated::ProofAuthorizingKey { ak, nk }
    }
}
...
/// authority.
///
/// `ask` derives [`SpendValidatingKey`](super::private::SpendValidatingKey)
...
impl SpendAuthorizingKey {
    ...
    #[must_use]
    pub fn derive_auth_public(&self) -> private::SpendValidatingKey {
        // reddsa::VerificationKey::from(&signing_key) performs [sk]G
        // (scalar-times-basepoint), not a trivial type conversion.
        private::SpendValidatingKey(reddsa::VerificationKey::from(&self.0))
    }
