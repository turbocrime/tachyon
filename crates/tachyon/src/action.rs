...
use crate::{
    ...
    keys::{private::SpendValidatingKey, custody, public},
    note::Note,
    value,
};
...
pub enum Effect {
    /// Spend — signed with
    /// [`SpendAuthorizingKey::derive_action_private`](custody::SpendAuthorizingKey::derive_action_private).
    Spend,
    /// Output — signed via
    /// [`ActionSigningKey<Output>::sign`](custody::ActionSigningKey::sign).
    Output,
}
...
impl Plan {
    ...
    pub fn output(note: Note, theta: ActionEntropy, rcv: value::CommitmentTrapdoor) -> Self {
        ...
        let rsk = custody::ActionSigningKey::new(alpha);
